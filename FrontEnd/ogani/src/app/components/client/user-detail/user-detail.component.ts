...
export class UserDetailComponent implements OnInit {
  ...
  timestamp = Date.now();
  backendUrl = 'https://hgr0a62zxby.sn.mynetname.net:2003'; // Thêm base URL của backend
  ...
  getUser() {
    this.userService.getUser(this.username).subscribe({
      next: (res) => {
        ...
        if (
          res.profileImage &&
          (res.profileImage.startsWith('http') ||
            res.profileImage.startsWith('/api/'))
        ) {
          ...
          if (this.profileImageUrl && !this.profileImageUrl.includes('?')) {
            this.profileImageUrl =
              this.profileImageUrl + '?t=' + this.timestamp;
          }
  ...
  uploadImage(): void {
    if (this.selectedFile) {
      ...
      if (this.user && this.user.provider === 'google') {
        console.log(
          'User is Google user, replacing Google profile image with uploaded image'
        );
      }
          ...
          error: (err) => {
            ...
            if (err.error) {
              if (typeof err.error === 'string') {
                ...
                if (
                  err.error.message.includes('FileNotFoundException') ||
                  ...
                  this.messageService.add({
                    ...
                    summary: 'Gợi ý',
                    detail:
                      'Please contact administrator to create storage directory on server',
                  });
            ...
            if (err.status === 500) {
              this.messageService.add({
                ...
                summary: 'Thông tin',
                detail:
                  'This could be a server error, please try again later or contact administrator',
              });
            }
          },
        });
  ...
  onImageError(event: any): void {
    ...
    if (this.user && this.user.provider === 'google' && this.user.avatar) {
      console.log(
        'Attempting to use Google avatar URL directly:',
        this.user.avatar
      );
