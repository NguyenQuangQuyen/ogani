...
export class ReportApiService {
    ...
    downloadDashboardReport(options?: { title?: string; exportedBy?: string }): void {
        const params = new HttpParams()
            .set('title', options?.title || 'Báo cáo tổng quan hệ thống')
            .set('exportedBy', options?.exportedBy || 'admin');

        this.http.get(`${this.REPORT_API}/dashboard`, { params, responseType: 'blob' as const })
            .subscribe({
                next: (blob) => {
    ...
    downloadDashboardReportSync(payload: any): void {
        this.http.post(`${this.REPORT_API}/dashboard-sync`, payload, { responseType: 'blob' as const })
            .subscribe({
                next: (blob) => {
                    const url = window.URL.createObjectURL(blob);
                    const a = document.createElement('a');
